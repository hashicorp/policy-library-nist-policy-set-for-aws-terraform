...
This rule is covered by the [elb-ensure-valid-desync-mitigation-mode](https://github.com/hashicorp/policy-library-NIST-Policy-Set-for-AWS-Terraform/blob/main/policies/elb/elb-ensure-valid-desync-mitigation-mode.sentinel) policy with the `classic` as the `lb_type` parameter.
